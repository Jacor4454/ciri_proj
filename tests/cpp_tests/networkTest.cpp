...
TEST_F(NetworkTest, NetworkFullLearn) {
    ...
    std::ifstream f("./tests/testData/grids.txt");
    ...
    std::ifstream f2("./tests/testData/labels.txt");
    if(!f2) throw std::runtime_error("cannot open file");
    int ind;
    for(int i = 0; i < n; i++){
        f2 >> ind;
    ...
    Network MNISTNetwork(
        inputDefObject({1,784}), 
        {
            layerDefObject({1,100}, layers::perceptron, activations::ReLU, new AdagradLearnerSelector(0.01))
        }, 
        outputDefObject({1,10}, layers::perceptron, activations::Sigmoid, errors::MSE, new AdagradLearnerSelector(0.01))
        , 1, 1231
    );
